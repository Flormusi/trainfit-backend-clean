...
class CronService {
     ...
     */
    static schedulePaymentReminders() {
     ...
     */
    static async checkAndSendPaymentReminders() {
        try {
     ...
     */
    static async processPaymentReminder(subscription) {
        try {
            ...
            const emailSent = await this.sendPaymentReminderEmail(user.email, user.clientProfile?.name || user.name || 'Cliente', trainerName, dueDate, reminderType, emailSubject);
     ...
     */
    static async sendPaymentReminderEmail(clientEmail, clientName, trainerName, dueDate, reminderType, subject) {
      ...
      `;
        ...
        return await emailService_1.EmailService.sendEmail({
            to: clientEmail,
            subject: subject,
            html: emailContent
        });
     ...
     */
    static getNotificationMessage(reminderType, daysDifference, dueDate) {
     ...
     */
    static async testPaymentReminders() {
