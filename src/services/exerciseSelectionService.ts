...
export interface ExerciseSelectionParams {
  objetivo: string;
  splitDay: number; // 1, 2 o 3
  nivel: 'principiante' | 'intermedio' | 'avanzado';
  genero?: 'masculino' | 'femenino' | 'unisex';
  dias: 2 | 3; // Número total de días del split
}
...
export interface SelectedExercise {
  ...
  imageUrl?: string;
  videoUrl?: string;
  // Campos calculados para la rutina
  sets: number;
  reps: string;
  weight: string;
  restTime: string;
  notes?: string;
  order: number;
  category: 'movilidad' | 'principal' | 'finisher';
  priority: string;
}
...
export const selectExercisesForDay = async (params: ExerciseSelectionParams): Promise<SelectedExercise[]> => {
  ...
  const muscleQuotas = getMuscleQuotasForDay(objetivo, splitType, splitDay);
  ...
  if (selectedExercises.length < 10) {
    ...
    const fillerExercises = await selectFillerExercises(
      remaining, 
      selectedExercises, 
      muscleQuotas, 
      rules, 
 ...
 */
const selectMobilityExercises = async (count: number, startOrder: number): Promise<SelectedExercise[]> => {
  try {
    ...
    if (exercises.length === 0) {
      logger.warn('No se encontraron ejercicios de movilidad, usando ejercicios básicos');
      return createDefaultMobilityExercises(count, startOrder);
    }
    
    return exercises.slice(0, count).map((exercise, index) => 
      mapExerciseToSelected(exercise, startOrder + index, 'movilidad', {
        sets: 1,
        reps: '10-15',
 ...
 */
const selectMainExercises = async (
  muscleQuotas: MuscleGroupQuota,
  rules: any,
  nivel: string,
  genero?: string,
  startOrder: number = 1
 ...
 */
const selectExercisesForMuscleGroup = async (
  muscleGroup: string,
  count: number,
  rules: any,
  nivel: string,
  ...
  const searchTerms = MUSCLE_GROUP_MAPPING[muscleGroup as keyof typeof MUSCLE_GROUP_MAPPING] || [muscleGroup];
  ...
  try {
    ...
    return selectedExercises.map((exercise, index) => 
      mapExerciseToSelected(exercise, index + 1, 'principal', {
 ...
 */
const prioritizeExercises = (exercises: any[], priorityOrder: string[]): any[] => {
  return exercises.sort((a, b) => {
    ...
    const isMultiA = isMultiarticular(a.name);
    const isMultiB = isMultiarticular(b.name);
 ...
 */
const getPriorityScore = (exercise: any, priorityOrder: string[]): number => {
  const exercisePriority = getExercisePriority(exercise.name);
 ...
 */
const selectFinisherExercises = async (count: number, startOrder: number): Promise<SelectedExercise[]> => {
  try {
    const exercises = await prisma.exercise.findMany({
      where: {
        OR: [
    ...
    return exercises.slice(0, count).map((exercise, index) => 
      mapExerciseToSelected(exercise, startOrder + index, 'finisher', {
        sets: 3,
        reps: '15-20',
        weight: 'Peso corporal',
        restTime: '45 seg'
 ...
 */
const selectFillerExercises = async (
  count: number,
  existingExercises: SelectedExercise[],
  muscleQuotas: MuscleGroupQuota,
  rules: any,
  nivel: string,
  genero?: string,
  ...
  const fillerExercises: SelectedExercise[] = [];
  ...
  for (let i = 0; i < count && fillerExercises.length < count; i++) {
    ...
    const additionalExercises = await selectExercisesForMuscleGroup(
      muscleGroup,
      1,
      rules,
      nivel,
 ...
 */
const mapExerciseToSelected = (
  exercise: any,
  order: number,
  category: 'movilidad' | 'principal' | 'finisher',
  params: { sets: number; reps: string; weight: string; restTime: string }
): SelectedExercise => {
  return {
    id: exercise.id,
    name: exercise.name,
    ...
    order,
    category,
    priority: getExercisePriority(exercise.name)
  };
};
 ...
 */
const createDefaultMobilityExercises = (count: number, startOrder: number): SelectedExercise[] => {
 ...
 */
const createDefaultFinisherExercises = (count: number, startOrder: number): SelectedExercise[] => {
  const defaultExercises = [
 ...
 */
const getEstimatedWeight = (rules: any, nivel: string): string => {
  const weightMap = {
    principiante: 'Ligero',
    intermedio: 'Moderado',
    avanzado: 'Pesado'
  ...
  return weightMap[nivel as keyof typeof weightMap] || 'Moderado';
};

/**
 * Obtener tiempo de descanso según reglas y nivel
 */
const getRestTime = (rules: any, nivel: string): string => {
